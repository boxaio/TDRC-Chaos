...
[t, x] = ode45(@(t,x)Lorenz63(x), tspan, [-0.5, 0.2, 1.5], opts);
...
transient_len = 1000;   % discard initial transient
train_len = 4000;       % training length
pred_len = 4000;        % prediction length (for autonomous prediction)
...
% initial condition
u0 = prediction(:, 1);
[t, ut] = ode45(@(t,x)Lorenz63(x), tt, u0, opts);
u_target = ut';
...
% the largest Lyapunov exponent of Lorenz63
le1 = 0.9016;
...
if compute_lyap
    ...
    num_step = 4 * num_inter_step;

    xr_ly = xr_pred;
    % initial condition
    u_0 = Wout * xr_ly;
    [~, u_ly] = ode45(@(t,x)Lorenz63(x), 0 : dt : nmax * num_step * dt, u_0, opts);
